...
from .util import GameOutcome, Move, get_livematch_id, get_win_target


class Team(models.Model):
    """
    A Slack team (i.e. workspace or server)
    """

    id = models.TextField(primary_key=True, help_text="Slack team ID")
    # TODO populate this during installation
    # name = models.TextField(help_text="Team name, as provided by Slack")


class Player(models.Model):
    """
    A user of the app. We use `Player` instead of `User` to prevent conflicts
    if we ever decide to enable the Django built-in `User` model.

    Right now, players are linked directly to Slack users. We might want to
    break this apart at some point, but right now it's fine. Hypothetically
    this is extensible to other services like Discord anyway.
    """
    ...
    id = models.TextField(primary_key=True, help_text="Slack user ID")
    name = models.TextField(help_text="User name, as provided by Slack")

    def slack_mention(self) -> str:
        """
        Get an escaped string to mention this player in Slack
        """
        return f"<@{self.id}>"
...
class LiveMatch(models.Model):
    """
    An ongoing match. This is *mostly* the same thing as `Match`, but
    ...
    def player_join(self, player):
        ...
        # If the match is full, they can't join
    ...
    def save_to_permanent(self, winner_id):
        """
        Save a `Match` object corresponding to this `LiveMatch`. This will put
        it in the permanent record books.
        """
